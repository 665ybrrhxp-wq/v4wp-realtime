...
================================================================
섹션 6: 시그널 감쇠 분석 (Signal Decay Tracking)
================================================================

- "시그널 감쇠 분석" 섹션이 사용자 프롬프트에 제공되면, 5거래일 추적 결과를 반영하라
- CONFIRMED: 시그널이 5일간 유지 → 기존 판단을 유지하거나 상향
- FADING: 시그널이 감쇠 중 → conviction을 한 단계 보수적으로 보정
- FALSE_POSITIVE: 시그널이 즉시 소멸 → 위양성 가능성을 명시적으로 경고, conviction을 2단계 하향
- Score 추이가 일관되게 하락하면 의장은 risk_note에 "시그널 약화" 언급
- 감쇠 분석이 없으면(새 시그널) 해당 섹션을 무시하라
...
{context['similar_signals_text']}"""
    ...
    # Signal Decay 컨텍스트 (5거래일 스코어 감쇠 분석)
    decay_section = ""
    if context and context.get('decay_context'):
        dc = context['decay_context']
        decay_class = dc['decay_class']
        class_kr = {'CONFIRMED': '확인됨(유지)', 'FADING': '감쇠 중', 'FALSE_POSITIVE': '위양성 의심'}
        trend_str = " → ".join(f"{s:.4f}" for s in dc.get('score_trend', []))
        decay_section = f"""

=== 시그널 감쇠 분석 (5거래일 추적) ===
감쇠 분류: {decay_class} ({class_kr.get(decay_class, decay_class)})
5일 평균 Score: {dc['score_5d_avg']:.4f}
Score 추이: {trend_str}"""
...
=== 최근 10일 스코어 추이 ===
{score_trend}{decay_section}{similar_section}{accuracy_section}
