"""
V4 Signal + 2x Leverage vs Pure DCA — Individual Stocks
========================================================
개별 종목에 V4 신호 시 2배 레버리지 효과를 시뮬레이션.

개별 종목에는 2x 레버리지 ETF가 없으므로,
"V4 신호 시 수익률을 2배로 계산"하는 합성(synthetic) 레버리지 방식 사용.

전략 A (V4+2x):
  - 매월 초 $500 입금
  - V4 매수 신호 시: 가용자금의 50%로 합성 2x 레버리지 매수
    (실제로는 같은 종목을 사되, 해당 포지션의 일간 수익률을 2배로 적용)
...
합성 2x 레버리지 계산:
  매일: leveraged_price[t] = leveraged_price[t-1] * (1 + 2 * daily_return[t])
  이는 실제 2x 레버리지 ETF의 일간 리밸런싱 구조를 그대로 모방.
  (변동성 감쇠 포함)
"""
...
def build_synthetic_2x(close):
    """Build synthetic 2x daily-rebalanced leveraged price series.
    Includes volatility decay and expense ratio, mimicking real leveraged ETFs."""
...
# ═══════════════════════════════════════════════════════════
print("=" * 115)
print("  V4 Signal + Synthetic 2x Leverage vs Pure DCA — Individual Stocks")
print(f"  Monthly: ${MONTHLY_DEPOSIT:.0f} | Signal: {SIGNAL_BUY_PCT:.0%} cash -> 2x | Month-end: rest -> 1x")
print(f"  Synthetic 2x: daily rebalanced, expense ratio {EXPENSE_RATIO_DAILY*252*100:.2f}%/yr")
print("=" * 115)
...
for tk, sector in TICKERS.items():
    ...
    # ── Simulate ──
    # A: V4 signal -> 2x, month-end -> 1x
    cash_a = 0.0
    shares_1x_a = 0.0  # regular shares
    shares_2x_a = 0.0  # synthetic 2x shares
    signal_count = 0
    ...
    val_2x = shares_2x_a * close_2x[-1]
    pct_2x = val_2x / final_a * 100 if final_a > 0 else 0
    ...
    price_change_2x = (close_2x[-1] / close_2x[0] - 1) * 100

    all_results.append({
        'ticker': tk, 'sector': sector,
        'period': f"{start_yr}-{end_yr}", 'n_years': n_years,
        'ann_vol': ann_vol,
        'price_1x': price_change_1x, 'price_2x': price_change_2x,
...
print(f"\n{'━' * 115}")
print(f"  RANKING: V4+2x Edge by Ticker (sorted by annual avg diff)")
print(f"{'━' * 115}")
print(f"  {'Rank':>4s} {'Ticker':<7s} {'Sector':<9s} {'Period':>10s} {'Vol':>5s} "
      f"{'Sigs':>5s} {'V4+2x Avg':>10s} {'DCA Avg':>10s} {'Diff':>9s} "
      f"{'Final V4+2x':>13s} {'Final DCA':>13s} {'$Diff':>12s} {'W/L/D':>7s}")
print(f"  {'─' * 113}")
...
# ═══════════════════════════════════════════════════════════
print(f"\n{'━' * 115}")
print(f"  ANALYSIS: Volatility vs V4+2x Edge")
print(f"{'━' * 115}")
...
for r in all_results[:5]:
    ...
    print(f"  {'Year':>6s} {'V4+2x':>10s} {'DCA':>10s} {'Diff':>9s} {'Sigs':>5s}")
    print(f"  {'─' * 48}")
...
# ═══════════════════════════════════════════════════════════
print(f"\n{'━' * 115}")
print(f"  RISK: Worst Year Drawdown (V4+2x vs DCA)")
print(f"{'━' * 115}")
print(f"  {'Ticker':<7s} {'Worst Year':>10s} {'V4+2x':>10s} {'DCA':>10s} {'Extra Loss':>12s}")
print(f"  {'─' * 55}")
