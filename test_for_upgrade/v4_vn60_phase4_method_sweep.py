"""
VN60 Phase 4: Score Combination Method Sweep
=============================================
현재 가중합(0.60×F + 0.40×D)을 대체할 합산 방식 비교

방식 A. Sequential Gate: S_Div 셋업 → S_Force 트리거
방식 B. AND Gate: 두 지표 동시 양수
방식 C. Div-Gate: S_Div가 S_Force의 자격 부여

총 10가지 조합 × 14 티커
고정: MACD(12/26/9), FN/2.0, DivClip/3.0 (Phase 1~3 결론)
"""
...
DIV_CLIP = 3.0; FORCE_NORM = 2.0
MACD_FAST = 12; MACD_SLOW = 26; MACD_SIGNAL = 9

# 10가지 합산 방식
METHODS = [
    {"key": "CUR",     "grp": "기준",  "desc": "가중합 60/40 (현재)"},
    {"key": "SEQ-10",  "grp": "A.Seq", "desc": "Sequential w=10d th=0.15"},
    {"key": "SEQ-20",  "grp": "A.Seq", "desc": "Sequential w=20d th=0.15"},
    {"key": "SEQ-30",  "grp": "A.Seq", "desc": "Sequential w=30d th=0.15"},
    {"key": "AND-MUL", "grp": "B.AND", "desc": "곱셈 (F×D)"},
    {"key": "AND-MIN", "grp": "B.AND", "desc": "최소값 min(F,D)"},
    {"key": "AND-GEO", "grp": "B.AND", "desc": "기하평균 √(F×D)"},
    {"key": "DG-0.1",  "grp": "C.DG",  "desc": "DivGate th=0.1"},
    {"key": "DG-0.2",  "grp": "C.DG",  "desc": "DivGate th=0.2"},
    {"key": "DG-0.3",  "grp": "C.DG",  "desc": "DivGate th=0.3"},
]
CURRENT_KEY = "CUR"
...
# ═══════════════════════════════════════════════════════════
def combine_current(sf, sd):
    """현재: 가중합"""
    return 0.60 * sf + 0.40 * sd


def combine_sequential(sf, sd, window, div_th):
    """A. Sequential Gate: S_Div 셋업 → S_Force 트리거
    최근 window일 내 S_Div > div_th 발생했으면 S_Force를 score로 사용"""
    sd_series = pd.Series(sd)
    rolling_max = sd_series.rolling(window + 1, min_periods=1).max().values
    return np.where(rolling_max > div_th, sf, 0.0)


def combine_and_mul(sf, sd):
    """B-1. AND 곱셈: 둘 다 양수일 때 F×D"""
    both_pos = (sf > 0) & (sd > 0)
    return np.where(both_pos, sf * sd, 0.0)


def combine_and_min(sf, sd):
    """B-2. AND 최소값: 둘 다 양수일 때 min(F,D)"""
    both_pos = (sf > 0) & (sd > 0)
    return np.where(both_pos, np.minimum(sf, sd), 0.0)


def combine_and_geo(sf, sd):
    """B-3. AND 기하평균: 둘 다 양수일 때 sqrt(F×D)"""
    both_pos = (sf > 0) & (sd > 0)
    return np.where(both_pos, np.sqrt(sf * sd), 0.0)


def combine_dgate(sf, sd, div_th):
    """C. Div-Gate: S_Div 수준에 따라 S_Force 사용/감쇠/차단
    S_Div > div_th → score = S_Force (전적 신뢰)
    0 < S_Div ≤ div_th → score = 0.5 × S_Force (반만 신뢰)
    S_Div ≤ 0 → score = 0 (차단)"""
    high = sd > div_th
    mid = (sd > 0) & (~high)
    return np.where(high, sf, np.where(mid, sf * 0.5, 0.0))


def get_score(sf, sd, method_key):
    """방식별 스코어 계산"""
    if method_key == "CUR":
        return combine_current(sf, sd)
    elif method_key == "SEQ-10":
        return combine_sequential(sf, sd, 10, 0.15)
    elif method_key == "SEQ-20":
        return combine_sequential(sf, sd, 20, 0.15)
    elif method_key == "SEQ-30":
        return combine_sequential(sf, sd, 30, 0.15)
    elif method_key == "AND-MUL":
        return combine_and_mul(sf, sd)
    elif method_key == "AND-MIN":
        return combine_and_min(sf, sd)
    elif method_key == "AND-GEO":
        return combine_and_geo(sf, sd)
    elif method_key == "DG-0.1":
        return combine_dgate(sf, sd, 0.1)
    elif method_key == "DG-0.2":
        return combine_dgate(sf, sd, 0.2)
    elif method_key == "DG-0.3":
        return combine_dgate(sf, sd, 0.3)


# ═══════════════════════════════════════════════════════════
# Signal & Simulation (Phase 1~3과 동일)
# ═══════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════
print("=" * 130)
print("  VN60 PHASE 4: Score Combination Method Sweep")
print("  전략: 월 $500 → 시그널 시 50% 레버리지(2x/3x) 매수 → 월말 잔여금 1x 매수")
print("  고정: MACD(12/26/9), FN/2.0, DivClip/3.0")
print("  비교: 가중합 vs Sequential Gate vs AND Gate vs Div-Gate")
print("=" * 130)
...
# Run All Methods
# ═══════════════════════════════════════════════════════════
print("  전체 방식 시뮬레이션 실행...")
master = {}  # master[method_key] = {ticker: result}
...
for method in METHODS:
    ...
    for tk in tks:
        td = ticker_data[tk]
        score_arr = get_score(td['s_force'], td['s_div'], mk)
        buys = get_buy_signals(td['df_s'], score_arr)
    ...
    sig_yr = np.nanmean([results[tk]['total_sigs'] / max(results[tk]['n_years'], 1) for tk in tks])
    print(f"    {mk:<8s}  2x={avg_2x:>+6.2f}%  3x={avg_3x:>+6.2f}%  DCA={avg_dca:>+6.2f}%  sig/yr={sig_yr:.1f}")


# ═══════════════════════════════════════════════════════════
# Rankings
# ═══════════════════════════════════════════════════════════
ranking = sorted(METHOD_KEYS,
...
# ═══════════════════════════════════════════════════════════
print(f"\n{'=' * 140}")
print(f"  [1] 전체 방식 랭킹: 14 티커 평균 연평균수익률")
print(f"{'=' * 140}")

print(f"\n  {'Rank':>4s} {'Method':<8s} {'Grp':<6s} {'Description':<24s}"
      f" │ {'VN60+2x':>9s} {'VN60+3x':>9s} {'DCA':>9s}"
      f" │ {'Edge2x':>8s} {'Eff2x':>7s}"
      f" │ {'Hit90':>6s} {'Fwd90':>8s} {'Sig/yr':>7s}"
      f" │ {'Note':>12s}")
print(f"  {'=' * 130}")

for i, mk in enumerate(ranking):
    m = master[mk]
    md = next(x for x in METHODS if x["key"] == mk)
    ...
    note = ""
    if mk == CURRENT_KEY: note = "◀ CURRENT"
    elif i == 0: note = "★ BEST"

    print(f"  {i+1:>4d} {mk:<8s} {md['grp']:<6s} {md['desc']:<24s}"
...
# ═══════════════════════════════════════════════════════════
print(f"\n{'=' * 140}")
print(f"  [2] 방식별 상세 메트릭 비교")
print(f"{'=' * 140}")

print(f"\n  {'Metric':<20s}", end="")
for mk in METHOD_KEYS:
    tag = "◀" if mk == CURRENT_KEY else " "
    print(f" {mk+tag:>9s}", end="")
print()
print(f"  {'-' * (20 + 10 * len(METHOD_KEYS))}")
...
# ═══════════════════════════════════════════════════════════
# [3] 방식 그룹별 BEST: A vs B vs C
# ═══════════════════════════════════════════════════════════
print(f"\n{'=' * 140}")
print(f"  [3] 방식 그룹별 최적 조합")
print(f"{'=' * 140}")

groups = {"기준": [], "A.Seq": [], "B.AND": [], "C.DG": []}
for md in METHODS:
    groups[md["grp"]].append(md["key"])

print(f"\n  {'Group':<8s} {'Best Method':<10s} {'2x':>9s} {'3x':>9s} {'Edge2x':>8s} {'Hit90':>7s} {'Fwd90':>8s} {'Sig/yr':>7s}")
print(f"  {'=' * 75}")

group_bests = {}
for grp, keys in groups.items():
    best_in_grp = max(keys, key=lambda k: np.nanmean([master[k][tk]['avg_2x'] for tk in tks]))
    group_bests[grp] = best_in_grp
    m = master[best_in_grp]
    a2 = np.nanmean([m[tk]['avg_2x'] for tk in tks])
    a3 = np.nanmean([m[tk]['avg_3x'] for tk in tks])
    e2 = np.nanmean([m[tk]['edge_2x'] for tk in tks])
    hr = np.nanmean([m[tk]['hit_rate_90'] for tk in tks])
    fw = np.nanmean([m[tk]['avg_fwd_90'] for tk in tks])
    sy = np.nanmean([m[tk]['total_sigs'] / max(m[tk]['n_years'], 1) for tk in tks])
    print(f"  {grp:<8s} {best_in_grp:<10s} {a2:>+8.2f}% {a3:>+8.2f}% {e2:>+7.2f}% {hr:>6.1f}% {fw:>+7.1f}% {sy:>6.1f}")


# ═══════════════════════════════════════════════════════════
# [4] 티커별 연평균수익률: BEST vs CURRENT
# ═══════════════════════════════════════════════════════════
print(f"\n{'=' * 140}")
print(f"  [4] 티커별 연평균수익률: BEST({best_key}) vs CURRENT(CUR)")
print(f"{'=' * 140}")
...
# ═══════════════════════════════════════════════════════════
print(f"\n{'=' * 140}")
print(f"  [5] 시그널 품질: BEST({best_key}) vs CURRENT(CUR)")
print(f"{'=' * 140}")
...
# ═══════════════════════════════════════════════════════════
print(f"\n{'=' * 140}")
print(f"  [6] 최종 자산가치: BEST({best_key}) vs CURRENT(CUR)")
print(f"{'=' * 140}")
...
for target in ['QQQ', 'VOO']:
    ...
    print(f"\n{'=' * 140}")
    print(f"  [7] {target} 연도별: CURRENT(CUR) vs BEST({best_key})")
    print(f"{'=' * 140}")
...
# ═══════════════════════════════════════════════════════════
print(f"\n{'=' * 140}")
print(f"  [8] 시장 국면별 Edge(2x vs DCA): BEST({best_key}) vs CURRENT(CUR)")
print(f"{'=' * 140}")
...
# ═══════════════════════════════════════════════════════════
print(f"\n{'=' * 140}")
print(f"  [9] 방식별 티커 Edge(2x vs DCA) 히트맵 (%p)")
print(f"{'=' * 140}")
...
for mk in METHOD_KEYS:
    ...
    print(f" {mk+tag:>9s}", end="")
print(f"  {'Best':>8s}")
print(f"  {'-' * (7 + 10 * len(METHOD_KEYS) + 10)}")

for tk in tks:
    print(f"  {tk:<7s}", end="")
    best_m = max(METHOD_KEYS, key=lambda k: master[k][tk]['edge_2x'])
    for mk in METHOD_KEYS:
        e = master[mk][tk]['edge_2x']
        print(f" {e:>+8.2f}%", end="")
    print(f"  {best_m:>8s}")
...
print(f"  {'AVG':<7s}", end="")
for mk in METHOD_KEYS:
    e = np.nanmean([master[mk][tk]['edge_2x'] for tk in tks])
    print(f" {e:>+8.2f}%", end="")
best_avg_m = max(METHOD_KEYS, key=lambda k: np.nanmean([master[k][tk]['edge_2x'] for tk in tks]))
print(f"  {best_avg_m:>8s}")

# 티커별 최적 방식 분포
print(f"\n  티커별 최적 방식 분포:")
from collections import Counter
best_per_tk = [max(METHOD_KEYS, key=lambda k: master[k][tk]['edge_2x']) for tk in tks]
cnt = Counter(best_per_tk)
for mk in METHOD_KEYS:
    c = cnt.get(mk, 0)
    bar = "█" * c
    print(f"    {mk:<8s}: {c:>2d}개 {bar}")
...
# ═══════════════════════════════════════════════════════════
print(f"\n{'=' * 140}")
print(f"  GRAND SUMMARY: Phase 4 결론 — Score 합산 방식 비교")
print(f"{'=' * 140}")

print(f"\n  [A] 전체 방식 요약 (연평균수익률 2x 순):")
for mk in ranking:
    m = master[mk]
    md = next(x for x in METHODS if x["key"] == mk)
    a2 = np.nanmean([m[tk]['avg_2x'] for tk in tks])
    ...
    hr = np.nanmean([m[tk]['hit_rate_90'] for tk in tks])
    sy = np.nanmean([m[tk]['total_sigs'] / max(m[tk]['n_years'], 1) for tk in tks])
    tag = " ◀ CURRENT" if mk == CURRENT_KEY else ""
    print(f"    {mk:<8s} ({md['grp']})  2x={a2:>+.2f}%  edge={e2:>+.2f}%  hit90={hr:.1f}%  sig/yr={sy:.1f}{tag}")

print(f"\n  [B] 그룹별 최적:")
for grp, best_mk in group_bests.items():
    m = master[best_mk]
    a2 = np.nanmean([m[tk]['avg_2x'] for tk in tks])
    e2 = np.nanmean([m[tk]['edge_2x'] for tk in tks])
    hr = np.nanmean([m[tk]['hit_rate_90'] for tk in tks])
    print(f"    {grp:<8s} → {best_mk:<8s}  2x={a2:>+.2f}%  edge={e2:>+.2f}%  hit90={hr:.1f}%")
...
print(f"""
  ...
  +-------------------------------------------------------------------------+
  |  CURRENT (가중합 60/40)   {ca2:>+7.2f}%   {ca3:>+7.2f}%   {cad:>+7.2f}%   {ca2-cad:>+6.2f}%       |
  +-------------------------------------------------------------------------+
  |  BEST ({best_key:<8s})          {na2:>+7.2f}%   {na3:>+7.2f}%   {nad:>+7.2f}%   {na2-nad:>+6.2f}%       |
  +-------------------------------------------------------------------------+
